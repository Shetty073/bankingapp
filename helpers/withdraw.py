...
def withdraw_amount(amount, window):
    try:
        amount = float(amount)
    except:
    ...
    if current_balance > amount:
        ...
        if cursor.rowcount == None or cursor.rowcount < 1:
            ...
            track_transaction("withdrawed", f"-{amount}", current_balance, new_balance)
            display_alert(window, "Success!", "Amount withdrawn successfully")
    else:
        display_alert(window, "Error", "Cannot withdraw amount due to low balance")
